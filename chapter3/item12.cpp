...
/*
 * todo 饿汉式 废弃
 *  C++11标准以前大家探讨了很多种实现方式,包括下面的饿汉式方式
...
/*
 * todo 为了规避用户手动释放内存,可以提供一个辅助类帮忙回收内存. 并将单例类的析构函数写为私有
 * */
...
private:
    ...
    /*
     * todo 定义友元类, 通过友元类调用该类析构函数
     *  SafeDeletor 可以访问 SingleAutoSafe的私有成员.
     * */
...
/*
 * todo 但是上面的代码存在危险,比如懒汉式的使用方式,当多个线程调用单例时,有一个线程加锁进入3处的逻辑.
 ...
 *  为解决这个问题,C++11推出了std::call_once函数保证多个线程只执行一次
 *
 *
 * todo allocate 最原始 开辟空间 指定字节数.
 *      malloc   开辟一片连续的空间,但是不调用构造函数
 *      new      开辟一片连续的空间,并且调用构造函数
 * */
...
public:
    ...
    {
        /*
         * todo C++11标准 局部静态变量,多个线程调用也只会初始化一次.
         * */
